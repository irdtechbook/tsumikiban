...
@<href>{https://jsprimer.net/, https://jsprimer.net/}
...
//image[codesandbox][CodeSandboxの画面]
...
@<code>{getUserMedia}メソッドを利用すると、動画や音声が含まれるMediaStreamというストリームが返ってきます。このストリームをあらかじめ用意したvideo要素にわたすと、カメラで撮影している映像が再生されます。

//listnum[chap-smart-phone-000][][html]{
<html>
...
//listnum[chap-smart-phone-001][][html]{
<video playsinline id="video"></video>
...
Canvasには描写されている画像データを取得する@<code>{getImageData}というメソッドと、事前に用意した画像データを反映する@<code>{pushImageData}というメソッドが用意されています。
...
たとえば、dataが@<code>{[0,0,0,0, 255, 255, 255, 255]}だった場合、2ピクセルのデータで、ひとつ目のピクセルが@<code>{R: 0, B: 0, G: 0, A: 0}（透明な黒）、ふたつめのピクセルが@<code>{R: 255, B: 255, G: 255, A: 255}（不透明な白）であることを示しています。


ImageDataを使った例を示します。
この例では幅256px、高さ256pxのCanvas要素に対して、ランダムでRed（R: 255, B: 0, G: 0）・Blue（R: 0, B: 255, G: 0）・Green（R: 0, B: 255, G: 0）のうちの1色を各ピクセルに描写しています。
@<code>{getImageData}でCanvasのImageDataを取り出し、for文で各ピクセルに対して色の加工を行っています。

//listnum[chap-smart-phone-002][][html]{
...
端末の向きを利用したい場合は@<code>{DeviceOrientationEvent}、端末の動きを利用したい場合は@<code>{DeviceMotionEvent}というイベントが用意されています。
...
//listnum[chap-smart-phone-003][][js]{
...
記事に制作のポイントをまとめた@<fn>{qiita}ので興味がある方はご覧ください。

//footnote[qiita][スマートフォンを楽器にしてみた @<href>{https://qiita.com/mottox2/items/f54eadc38e98586909a2, https://qiita.com/mottox2/items/f54eadc38e98586909a2}]
...
//listnum[chap-smart-phone-004][][js]{
<div id="result"></div>
<div id="status"></div>
<script>
...
端末の向き、動作に関してはGoogle Developersにある記事@<fn>{device}がわかりやすいです。
イベントで取得できるデータが図付きで解説されているので、実際にコードを書いてみたいと思った方は目を通すといいでしょう。

//footnote[device][端末画面の向きと端末のモーション @<href>{https://developers.google.com/web/fundamentals/native-hardware/device-orientation, https://developers.google.com/web/fundamentals/native-hardware/device-orientation}]
...
@<href>{https://caniuse.com/, https://caniuse.com/}
